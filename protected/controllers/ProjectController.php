...
class ProjectController extends Controller
{
	...
	public function actionChoose()
	{
		...
		$dependency = new CDbCacheDependency('SELECT name FROM {{project}} WHERE project_id=:project_id');
		$dependency->params = array(
			':project_id' => $project_id
		);
		$project = Project::model()->cache(300, $dependency)->findByPk($project_id);
	...
	public function actionIndex($archived = 0)
	{
		...
		$this->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/plug-in/fprogress-bar/fprogress-bar.js');
		$this->clientScript->registerCssFile(Yii::app()->baseUrl . '/js/plug-in/fprogress-bar/fprogress-bar.css');
		$this->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/plug-in/jquery-gantt/js/jquery.fn.gantt.min.js');
		$this->clientScript->registerCssFile(Yii::app()->baseUrl . '/js/plug-in/jquery-gantt/style.css');
		Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/plug-in/jquery-form/jquery.form.min.js');
	...
	protected function getHomePageHtml($data)
	{
		...
		return empty($url) ? '<img alt="" src="' . Yii::app()->theme->baseUrl . '/images/icons/none16.png" />' : CHtml::link('<img alt="" src="' . Yii::app()->theme->baseUrl . '/images/icons/html.png" />', $url, array(
			'target' => '_blank'
		));
	}
	...
	{
		$url = $this->createUrl('/project/choose', array(
			'rr' => '',
			'menu_project_id' => $data->project_id
		));
		return CHtml::link('Switch to', $url, array(
			'class' => 'bkButtonBlueSmall'
		));
	}
	...
	public function actionManagePeople()
	{
		...
		$this->renderPartial('_inviteUsersForm', array(
			'model' => $model
		), false, true);
	}

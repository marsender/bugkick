...
class RegistrationController extends Controller
{
	...
	protected function fbRegisterUser($response)
	{
		...
		$user->password = $user->hashPassword($response['registration']['password']);
	...
	public function actionIndex()
	{
		...
		if (isset($_POST['User'])) {
			 ...
			 */
			if ($user->validate() && ($company = Company::model()->createNew($company)) !== null) {
				$user->save();
				$company->owner_id = $user->user_id;
				$company->save();
				$user->setDefaultEmailPreferences();
				...
				if ($userByCompany->save()) {
					...
					Yii::app()->user->setFlash('success', 'Registration completed. You can sign in with your credentials');
					$this->session['fromRegistration'] = 1;
					$loginForm = new LoginForm();
	...
	protected function handleImage($for, $fileName, $filePath)
	{
		...
		case 's3':
			...
			$s3FilePath = Storage::get('s3')->upload($bucket, $fileName, $filePath);
			@unlink($filePath);
			if (!empty($s3FilePath))
				return $s3FilePath;
			break;
	...
	public function actionVerify()
	{
		...
		$user = User::model()->find('registration_token=:registration_token', array(
	...
	public function performAjaxValidation($user, $company)
	{
		...
		if (isset($_POST['ajax']) && $_POST['ajax'] === 'user-form') {
			...
			$user->attributes = $this->request->getPost(get_class($user), array());
	...
	protected function validateCoupon($model)
	{
		...
		if (isset($_POST['Company']['coupon_id'])) {
			...
			if (!empty($couponCode)) {
				$coupon = Coupon::model()->enabled()->find('code=:code', array(
					':code' => $couponCode
				));
				if (empty($coupon))
					$errors[CHtml::activeId($model, 'coupon_id')] = array(

...
;
	...
	public function actionView()
	{
		...
		Yii::app()->clientScript->registerScript(null, "SyntaxHighlighter.all({toolbar:'false'});
", CClientScript::POS_END);
		...
		if ($id == Yii::app()->user->id) {
			...
			MixPanel::instance()->registerEvent(MixPanel::PROFILE_PAGE_VIEW, array(
	...
	public function actionDelete($id)
	{
		if (Yii::app()->request->isPostRequest) {
			...
			$user = $this->loadModel($id);
			if (!empty($user)) {
				UserByProject::model()->deleteAll('user_id=:user_id', array(
					':user_id' => $id
				));
				UserByGroup::model()->deleteAll('user_id=:user_id', array(
					':user_id' => $id
				));
				BugByUser::model()->deleteAll('user_id=:user_id', array(
					':user_id' => $id
				));
	...
	public function actionInvite()
	{
		...
		if (!empty($_POST['User']['email'])) {
			...
			if ($isUserInvited) {
				Yii::app()->user->setFlash('success', Yii::t('main', 'User has been invited'));
			}
		}
		...
		elseif (strstr(Yii::app()->request->getUrlReferrer(), '/new')) {
			...
			$this->redirect(Yii::app()->createUrl('new/2', array(
				'completed_step_2' => 1
			)));
		}
		else {
	...
	public function actionReinvite()
	{
		if (isset($_GET["id"]) && !empty($_GET["id"])) {
			...
			$acceptUrl = Yii::app()->createAbsoluteUrl('user/confirmInvite', array(
				't' => $inviteToken
			));

			$mailSubject = '[' . Yii::app()->name . '] ';
			$mailSubject .= Yii::app()->user->name . ' invites you to join the team for "' . Yii::app()->user->company_name . '"';
			$mailMessage = $this->renderFile(Yii::getPathOfAlias('application.views.mailTemplate.inviteMember') . '.php', array(
				'acceptUrl' => $acceptUrl
			), true);

			if ($model->save()) {
				$SESMail = new SESMail();
				$SESMail->send($model->email, Yii::app()->params['adminEmail'], $mailSubject, $mailMessage);
			}
	...
	protected function setNewPassword($user)
	{
		...
		if (!empty($attributes)) {
			...
			if ($model->validate()) {
				...
				$user->password = $user->hashPassword($model->password1);
				if ($user->save()) {
	...
	{
		if (Filter::model()->deleteAll('user_id=:user_id AND filter_id=:filter_id', array(
			'user_id' => User::getId(),
			'filter_id' => $id
		)) > 0) {

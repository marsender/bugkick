 ...
 // For storing search keywords
	...
	public function actionUpdateAjaxComment()
	{
		...
		if (isset($comment)) {
			...
			$p->options = array(
				'HTML.AllowedElements' => 'p,div,span,ul,ol,li,a,hr,pre,br,h1,h2,h3,h4,h5,h6,b,i,u,strike,big,small',
				'HTML.AllowedAttributes' => 'style,class,width,size,href, align'
			);
			$model->message = $p->purify($model->message);
			$model->message = preg_replace('/({{{#!)([^\s]*)\s+((.|\s)+?)\s*(!#}}})/', '<pre class="language-$2">$3</pre>', $model->message);
			$model->message = preg_replace('/(<pre class=")(language-)(">)/', '$1highlight$3', $model->message);
			...
			if ($model->save()) {
				...
				$this->renderPartial('_commentsList', array(
	...
	public function actionView($id)
	{
		...
		Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/plug-in/jquery-syntaxhighlighter/scripts/shBrushPhp.js');
...
, CClientScript::POS_END);
		...
		$bug = $this->bug = Bug::model()->resetScope()
			...
			->find('number=:number AND project_id=:project_id', array(
			':number' => $id,
...
, CClientScript::POS_END);
			...
			$this->render('view', array(
				...
				'users' => User::model()->bugRelated($bug)
					->findAll(),
				'tasks' => $tasks
			));
		}
	...
	protected function initBugForm()
	{
		...
		if ($this->bug->title != $this->bug->description) {
			$this->bug->description = str_replace('&#133;', '', $this->bug->title) . $this->bug->description;
		}
				...
				if ($hasBugCreated) {
					...
					if (strstr(Yii::app()->request->getUrlReferrer(), '/new')) {
						$result = htmlspecialchars(json_encode(array(
							...
							'redirect' => Yii::app()->createAbsoluteUrl('/bug', array(
								'completed_step_3' => 1
							))
						)), ENT_NOQUOTES);
					}
	...
	protected function updateRelatedToBug(BugBase $model, BugForm $form, $updateOnly = false)
	{
		...
		if ($updateOnly != 'assignee') {
			...
			if (!empty($values)) {
				$sql = 'INSERT INTO {{bug_by_label}} VALUES' . implode(',', $values);
		...
		if ($updateOnly != 'label') {
			...
			if (!empty($values)) {
				$sql = 'INSERT INTO {{bug_by_user}} VALUES' . implode(',', $values);
	...
	protected function createBug(BugBase $model, Project $project)
	{
		...
		try {
			...
			$prevBug = Bug::model()->resetScope()->findBySql('SELECT * FROM {{bug}} WHERE 1
			...
			if (empty($prevBug)) {
				...
				$globalPrevBug = Bug::model()->resetScope()->findBySql('SELECT * FROM {{bug}} WHERE 1
                    AND next_id = :zero
                    FOR UPDATE', array(
					':zero' => 0
				));
	...
	protected function onBugUpdateSuccess(Bug $model)
	{
		...
		if ($user->ticket_update_return == 1) {
			...
			if ($user->ticket_update_return == 2) {
				//stay on same ticket view page
				$this->redirect(Yii::app()->createUrl('bug/view/', array(
					'id' => $model->number
				)));
			}
	}

	public function updateBug(Bug $bugModel, $formId = null, $onSuccess = null)
	{
	...
	public function actionUpdate($id)
	{
		...
		$this->updateBug($bugModel, 'bug-update-form', array(
			$this,
			'onBugUpdateSuccess'
		));
		Yii::app()->end();
	...
	{
		return Bug::model()->resetScope()->findBySql('SELECT * FROM {{bug}} WHERE 1
                AND number = :number
                AND project_id = :project_id
                FOR UPDATE', array(
			':number' => $bugNumber,
	...
	public function actionDelete($id)
	{
		if (Yii::app()->request->isPostRequest) {
			...
			try {
				...
				//$project=Project::getCurrent();
				$prevBug = $this->getProjectBugByNumber($bug->prev_number, $bug->project_id);
				$nextBug = $this->getProjectBugByNumber($bug->next_number, $bug->project_id);
	...
	public function actionIndex()
	{
		...
		if (!Yii::app()->request->isAjaxRequest) {
			...
			//Edit Project Form
			Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl . '/js/project/index/common.js');
		}
		...
		if ($this->request->getParam('show') == 'calendar') {
			...
			if (!empty($bugs)) {
				foreach ($bugs as $key => $bug) {
					...
					$bugsArray[$key]['url'] = $this->createUrl('bug/view', array(
						'id' => $bug->number
					));
				}
			}
		...
		if (isset($filterID) && $filterID > 0) {
			$userFilter = Filter::model()->find('user_id=:user_id AND filter_id=:filter_id', array(
				'user_id' => User::getId(),
				'filter_id' => $filterID
			));
			if ($userFilter) {
		...
		$criteria->with = array(
			...
			'label' => !isset($labelsSQL) ? array() : array(
				'condition' => $labelsSQL . ' ' . $createdWithAPI . ' )',
				'together' => true
			),
			'user' => !isset($usersSQL) ? array() : array(
				'condition' => $usersSQL,
				'together' => true
			),
			'user',
		...
		$this->clientScript->registerScript(__CLASS__ . '#disable_selection', '$("body").attr("unselectable", "on").addClass("unselectable");', CClientScript::POS_READY);
		...
		if (Yii::app()->request->getParam('completed_step_3'))
			MixPanel::instance()->registerEvent(MixPanel::COMPLETE_STEP, array(
				'step' => 3
			)); // MixPanel events tracking
		elseif (Yii::app()->request->getParam('skip_step_3'))
			MixPanel::instance()->registerEvent(MixPanel::SKIP_STEP, array(
				'step' => 3
			)); // MixPanel events tracking
	...
	public function loadModel($id)
	{
		...
		$model = Bug::model()->resetScope()->find('number=:number AND project_id=:project_id', array(
			':number' => (int)$id,
			':project_id' => $project->project_id
		));
		if (empty($model))
	...
	public function actionPaginationUpdate()
	{
		...
		if (Yii::app()->request->isAjaxRequest) {
			...
			$this->renderPartial('_bugPagination', array(
				'pages' => $pages
			));
			Yii::app()->end();
		}
	...
	public function actionGetDuplicateFormByBugId()
	{
		if (Yii::app()->request->isAjaxRequest && !empty($_GET['id'])) {
			...
			if (!empty($model)) {
				...
				$this->layout = 'layout';
				$this->render('_duplicateForm', array(
					'model' => $form
				));
				Yii::app()->end();
			}
	...
	public function actionDndPrioritySort()
	{
		if (Yii::app()->request->isAjaxRequest) {
			...
			if ($element > $nextElement) {
				...
				$ticket = Bug::model()->findByAttributes(array(
					'priority_order' => $element
				));
				if ($ticket) {
			...
			InstantMessage::instance()->sendToAll(Project::getCurrent()->project_id, MessageType::TICKETS_ORDER_CHANGED);
			Yii::app()->end();
		}
		else
			throw new CHttpException(400, 'Invalid request.');
	...
	public function actionDndAddLabel()
	{
		...
		if (Yii::app()->request->isAjaxRequest && !empty($ticketIDs) && $labelID > 0) {
	...
	public function actionDndAddStatus()
	{
		...
		if (Yii::app()->request->isAjaxRequest && !empty($ticketIDs) && $statusID > 0) {

...
';
		$projects = Yii::app()->db->createCommand($sql)->queryAll(true, array(
		...
		return CHtml::dropDownList('menu_project_id', empty($selectedProject) ? null : $selectedProject->project_id, $data, array_merge(array(
...
';
	 ...
	 *
	 * @param mixed $response If var is a string, <br />
	...
	public function respond($response, $contentType = ResponseType::JSON)
	{
		...
		$_contentType = in_array($contentType, $contentTypes) ? $contentType : ResponseType::JSON;
		$mimeType = self::getMimeType($_contentType);
		header("Content-Type: {$mimeType}; charset=UTF-8");
		echo $_contentType == ResponseType::JSON ? CJSON::encode($response) : $response;
		Yii::app()->end();
	}

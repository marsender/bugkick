...
$this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns' => array(
		array(
			...
			'value' => function ($data)
			{
				$url = Yii::app()->createUrl('/project/manage', array(
					'project_id' => $data->project_id
				));
				return CHtml::link(Yii::t('main', 'Manage'), $url, array(
					'class' => 'bkButtonBlueSmall manageProject'
				));
			},
			'htmlOptions' => array(
			...
			'template' => '{archive} &nbsp; {update} &nbsp; {delete}',
			'buttons' => array(
				'update' => array(
					'click' => 'editProject'
				),
				'archive' => array(
					'label' => ($projectView == 'archived') ? Yii::t('main', 'Unarchive') : Yii::t('main', 'Archive'),
					'imageUrl' => ($projectView == 'archived') ? Yii::app()->theme->baseUrl . '/images/icons/unarchive-icon.png' : Yii::app()->theme->baseUrl . '/images/icons/archive-icon.png',
					'url' => 'Yii::app()->createUrl("/project/setarchived", array("id"=>$data->project_id))'
				),
				'delete' => array(
					'click' => 'Yii::app()->createUrl("/project/deleteproject", array("id"=>$data->project_id))'
				)
			)
		)
	)

...
class ExtMinScript extends CApplicationComponent
{
	...
	/**
	 * Set the minScript groupMap.
	 * This method needs to be executed before the component is initialized.
	 *
	...
	/**
	 * Generate Yii's scriptMap from supplied minScript groups or file paths.
	 * The supplied groups or file paths
	...
	public function generateUrl($generationValues)
	{
		...
		$basePath = dirname(Yii::app()->getRequest()->getScriptFile());
		foreach ($paths as $path) {
			...
			if ($filemtime !== false) {
				...
				Yii::log('Can\'t access file ' . $fullPath, CLogger::LEVEL_WARNING, 'ext.minScript.components.ExtMinScript');
			}
		}
		if (!empty($groups)) {

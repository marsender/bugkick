...
class UserController extends AdminController
{
	...
	protected function handleGridAjaxRequest(AUser $user, AUser $userRecent, $daysCount = 30)
	{
		...
		if (!empty($attributes)) {
			if ($ajaxVar == self::GRID_USERS_ID) {
				...
				if ($ajaxVar == self::GRID_RECENT_USERS_ID) {
	...
	public function actionUpdate($id)
	{
		...
		if (!empty($attributes) && $this->updateUser($user, $attributes)) {
			Yii::app()->end();
		}
		else {
			$user->password = '';
	...
	public function actionDelete($id)
	{
		if (Yii::app()->request->isPostRequest) {
			...
			UserByCompany::model()->deleteAll('user_id=:user_id', array(
				':user_id' => $id
			));
			UserByEmailPreference::model()->deleteAll('user_id=:user_id', array(
				':user_id' => $id
			));
			SettingsByUser::model()->deleteAll('user_id=:user_id', array(
				':user_id' => $id
			));

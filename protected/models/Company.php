...
class Company extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array(
				'company_logo, company_top_logo, api_key',
				'length',
				'max' => 255
			),
			array(
			...
			array(
				'company_id, owner_id, created_at, company_name, company_url, company_logo, company_top_logo, company_color, api_key, account_plan, show_ads, coupon_id, coupon_expires_at',
				'safe',
				'on' => 'search'
			)
		);
	...
	public function isGitHubIntegrationAvailable()
	{
		...
		$planConfig = StripePlanConfigFactory::createPlanConfig($this->account_plan, $storageType);
	...
	public function relations()
	{
		...
		return array(
			...
			'archivedProjectCount' => array(
				self::STAT,
				'Project',
				'company_id',
				'condition' => 'archived=1'
			),
			'activeProjectCount' => array(
				self::STAT,
				'Project',
				'company_id',
	...
	{
		if (empty(self::$_current_company) && isset(Yii::app()->user->company_id)) {
	...
	public static function getUserGroups($company_id = '')
	{
		...
		$criteria = new CDbCriteria();
		$criteria->with = array(
			'group' => empty($project) ? array() : array(
	...
	public static function getArchivedBugsCount($company_id = '')
	{
		...
		$rows = Yii::app()->db->createCommand()
			->from('{{bug}}', 'COUNT(*)')
			->where('company_id=:company_id AND isarchive=1', array(
			':company_id' => $company_id
		))
			->queryAll();
	...
	public static function getTodayBugsCount($company_id = '')
	{
		...
		$rows = Yii::app()->db->createCommand()
			->select('isarchive')
			->from('{{bug}}')
			->where('company_id=:company_id AND DATE(created_at) = DATE(NOW())', array(
			':company_id' => $company_id
		))
			->queryAll();
		return $rows;
	...
	public static function getRecentChangedBugs($company_id = '')
	{
		...
		$rows = Yii::app()->db->createCommand()
			...
			->from('{{bug}}')
			->where('company_id=:company_id AND (DATE(created_at) > date_sub(DATE(NOW()), INTERVAL 14 DAY) OR DATE(archiving_date) > date_sub(DATE(NOW()), INTERVAL 14 DAY)  )', array(
			':company_id' => $company_id
		))
			->queryAll();
		return $rows;
		...
		if (!empty($image)) {
			...
			case 'local':
				if (is_file('images/company_top_logo/' . $image)) {
					return ImageHelper::thumb($width, $height, 'images/company_top_logo/' . $image, $quality);
				}
				break;
			}
		}
		else {
			return ImageHelper::thumb($width, $height, 'images/logo.png', $quality);
		}
	}

...
class User extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array(
				'email',
				'application.extensions.validators.UniqueOrInvitedValidator',
				'on' => 'registration'
			),
			array(
				'email',
				'application.extensions.validators.DomainsBlacklistValidator',
				'on' => 'registration'
			),
			array(
				'encryption_algorithm, invited_by_id, default_page, email_notify, isadmin, is_global_admin, email_preference, hotkey_preference, userStatus, defaultAssignee, defaultCompany, defaultStatus, defaultLabel, tickets_per_page, ticket_update_return, use_wysiwyg, pro_status',
				'numerical',
				'integerOnly' => true
			),
			array(
			...
			array(
				'lname, randomPassword, registration_token, salt, look_and_feel, facebook_id',
				'length',
				'max' => 255
			),
			array(
				'created_at, inviteToken',
				'safe'
			),
			array(
				'email',
			...
			array(
				'current_project_id, github_user_id, github_auth_token',
				'default',
				'value' => null
			),
			array(
			...
			array(
				'encryption_algorithm',
				'default',
				'value' => self::ALGORITHM_SHA256
			),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array(
				'user_id, is_global_admin, pro_status, default_page, invited_by_id, facebook_id, created_at, name, lname, email, password, salt, email_notify, current_project_id, isadmin, profile_img, email_preference, hotkey_preference, look_and_feel, randomPassword, userStatus, defaultAssignee, defaultStatus, defaultLabel, use_wysiwyg',
				'safe',
				'on' => 'search'
			)
		);
	...
	public function lafExists($attribute, $params)
	{
		...
		if (!LookAndFeel::model()->exists('name=:name', array(
			':name' => $lafName
		)))
			$this->addError($attribute, "Look-and-Feel scheme '{$lafName}' doesn't exists");
	}
	...
	public function generateInviteToken()
	{
		...
		if ($this->exists('inviteToken=:token', array(
			':token' => $token
		))) {
	...
	public static function _getImageSrc($profile_img, $facebook_id, $width = 50, $height = 50, $quality = 75)
	{
		if (!empty($profile_img)) {
			...
			case 'local':
				if (is_file('images/profile_img/' . $width . '_' . $height . '_' . $profile_img)) {
					return ImageHelper::thumb($width, $height, 'images/profile_img/' . $width . '_' . $height . '_' . $profile_img, $quality);
				}
				break;
			}
		}
		if (empty($profile_img) && !empty($facebook_id)) {
			...
			return ImageHelper::thumb($width, $height, 'images/profile_img/default.jpg', $quality);
		}
	}
	...
	{
		return self::_getImageSrc($this->profile_img, $this->facebook_id, $width, $height, $quality);
	}
	...
	public function setDefaultEmailPreferences()
	{
		...
		foreach ($prefs as $pref) {
			UserByEmailPreference::model()->createForUserAndPreference($this, $pref, EmailPreference::STATE_ON);
		}
		return true;
	}
	 ...
	 */
	public static function isCompanyAdmin($company_id = 0)
	{
		$model = User::current();
		if (empty($model)) {
			return false;
		...
		if ($company_id == 0) {
			...
			//$company_id = empty($company) ? 0 : $company->company_id;
			if ($company_id == 0) {
				return false;
			}
		}
		$res = UserByCompany::model()->exists('user_id = :user_id AND company_id =:company_id AND is_admin = 1', array(
	...
	public static function isProjectAdmin($project_id = 0)
	{
		$model = User::current();
		if (empty($model)) {
			return false;
		...
		if ($project_id == 0) {
			...
			$project_id = empty($project) ? 0 : $project->project_id;
			if ($project_id == 0) {
				return false;
			}
		}
		$res = UserByProject::model()->exists('user_id = :user_id AND project_id = :project_id AND is_admin = 1', array(
			':user_id' => $model->user_id,
	...
	{
		$user = User::current();
		return empty($user) || empty($user->user_settings) ? null : $user->user_settings;
	}
	...
	public function generateSalt()
	{
		...
		return self::model()->exists('salt=:salt', array(
			':salt' => $salt
	...
	public function validatePassword($password)
	{
		...
		if ($isValid && $this->encryption_algorithm != RegistrationController::PASSWORD_ENCRYPTION_ALGORITHM) {
	...
	{
		$this->encryption_algorithm = RegistrationController::PASSWORD_ENCRYPTION_ALGORITHM;
	...
	{
		return Bcrypt::hash($password . $this->salt(), Yii::app()->params['bcryptWorkFactor']);
	}
	...
	{
		return UserByCompany::model()->updateAll(array(
			'user_status' => $status

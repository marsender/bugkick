...
/**
 * This constant defines whether the application should be in trace mode or not.
 * Defaults to false.
 */
			...
			if (($path = self::getPathOfAlias($namespace)) !== false) {
				...
				if ($forceInclude) {
					if (is_file($classFile))
						...
						throw new CException(Yii::t('yii', 'Alias "{alias}" is invalid. Make sure it points to an existing PHP file.', array(
							'{alias}' => $alias
						)));
				...
				throw new CException(Yii::t('yii', 'Alias "{alias}" is invalid. Make sure it points to an existing directory.', array(
					'{alias}' => $namespace
				)));
		}
		...
		if (isset(self::$_aliases[$alias]))
			...
			if (($pos = strpos($alias, '.')) !== false) {
				...
				if (isset(self::$_aliases[$rootAlias]))
					...
					if (self::$_app instanceof CWebApplication) {
